...
LiquidCrystalRus lcd(4, 5, 10, 11, 12, 13); //объявляется объект типа LiquidCrystalRus с определёнными параметрами (пины LCD -> Arduino)

SoftwareSerial       mySerial(2, 3);        //объявляется объект типа SoftwareSerial с параметрами (Rx, Tx). Можно указывать любые выводы, поддерживающие прерывание PCINTx
Adafruit_Fingerprint finger = Adafruit_Fingerprint(&mySerial);  // объявляем объект типа Adafruit_Fingerprint (ptrSerial). ptrSerial - ссылка на объект UART к которому подключен модуль отпечатка пальцев

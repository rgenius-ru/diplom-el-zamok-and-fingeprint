...
uint32_t  timeModeAccess = 0;// время установки флага fModeAccess указывающего о необходимости открытия замка
...
uint8_t   arrId[162];// массив найденных ID в базе

bool      fTemplateFun  = 0;// флаг указывающий на результат сохранения шаблона
bool      fModeAccess = 0;// флаг указывающий о необходимости открытия замка
bool      fStateKey  = 1;// флаг указывающий о состоянии работы замка
bool      fNeedUpdLCD = 1;// флаг указывающий о необходимости обновления информации на дисплее
...
void loop() {
  ...
  if (fNeedUpdLCD) {
    displayUpdate(); // если установлен флаг fNeedUpdLCD (нужно обновить информацию на дисплее), то вызываем функцию displayUpdate();
  }
...
void displayUpdate() {
  ...
  switch (modeMenu) {
    case  0: /* вне меню */
      myLCD.setCursor(0, 0); myLCD.print(F("ДОСТУП:")); myLCD.print(fModeAccess ? F("ОТКРЫТО") : F("ЗАКРЫТО" ) );
    ...
    case 39: /*10 уровень меню - сохранить ID    */ myLCD.setCursor(0, 0); myLCD.print(F("МЕНЮ >ДОБАВИТЬ ID ")); if (thisId < 100) {
      ...
      myLCD.setCursor(0, 1); myLCD.print(fTemplateFun ? F("Добавлено!") : F("Ошибка :("));
      break;
    ...
    case 49: /*10 уровень меню - удалить ID      */ myLCD.setCursor(0, 0); myLCD.print(F("МЕНЮ> УДАЛИТЬ ")); if (thisId == 255) {
      ...
      myLCD.setCursor(0, 1); myLCD.print(fTemplateFun ? F("Палец удален!") : F("Ошибка :("));
      break;
...
void Func_sensor_communication() {
  switch (modeMenu) {
    ...
    case  21: firstId = 0; countId = 0;                                    // сбрасываем количество найденных Id и номер первого среди отображенных
      ...
      } modeMenu = 22; fNeedUpdLCD = 1;                             // переходим в 3 уровень меню ( показать ID ) и устанавливаем флаг указывающий о необходимости обновить информацию на дисплее
      break;
    ...
    case  31: thisId = 161;                                                     // устанавливаем номер 161 для первого свободного ID в базе отпечатков
      ...
      } modeMenu = 32; fNeedUpdLCD = 1;                             // переходим в 3 уровень меню ( сохранить ID ) и устанавливаем флаг указывающий о необходимости обновить информацию на дисплее
      break;
    ...
    case  34: fTemplateFun = 0; fNeedUpdLCD = 1;                               // устанавливаем флаг указывающий о необходимости обновить информацию на дисплее
    ...
    case  35: delay(500); modeMenu = 36; fNeedUpdLCD = 1;                   // ждём 0.5 сек и пытаемся перейти в 7 уровень меню ( сохранить ID )
    ...
    case  37: fTemplateFun = 0; fNeedUpdLCD = 1;                               // устанавливаем флаг указывающий о необходимости обновить информацию на дисплее
    ...
    case  38: fTemplateFun = 0; modeMenu = 39; fNeedUpdLCD = 1;           // пытаемся перейти в 10 уровень меню ( результат сохранения ID ) и устанавливаем флаг указывающий о необходимости обновить информацию на дисплее
      ...
      break;
    case  39: delay(2000); fTemplateFun = 0; modeMenu = 30; fNeedUpdLCD = 1; // выходим из меню через 2 сек
      break;
    //            Если требуется удалить все ID из базы отпечатков
    case  42: fTemplateFun = 1; modeMenu = 49; fNeedUpdLCD = 1;           // устанавливаем результат и пытаемся перейти в 10 уровень меню ( результат удаления ID ) установив флаг указывающий о необходимости обновить информацию на дисплее
      for (uint8_t id = 0; id < 162; id++) {                                 // проход по 162 идентификаторам
        if (fSensor.loadModel(id) == FINGERPRINT_OK) {                        // если по указанному идентификатору найден шаблон отпечатка, то ...
    ...
    case  45: fTemplateFun = 0; modeMenu = 49; fNeedUpdLCD = 1;           // пытаемся перейти в 10 уровень меню ( результат удаления ID ) и устанавливаем флаг указывающий о необходимости обновить информацию на дисплее
      ...
      break;
    case  49: delay(2000); fTemplateFun = 0; modeMenu = 40; fNeedUpdLCD = 1; // выходим из меню через 2 сек
      break;
    default: break;
  }
}
